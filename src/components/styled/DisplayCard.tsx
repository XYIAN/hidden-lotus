...
import { Card, CardBody, CardImage, YogaCard } from './Card'
...
const CardContainer = styled(YogaCard)<{ cardSize: 'small' | 'medium' | 'large' }>`
  ${({ cardSize }) => {
    switch (cardSize) {
      case 'small':
        return css`
      ...
      <CardBody>
        ...
        {showType && type && (
          <TypeBadge type={type}>
            <span className="p-tag">
              <i className={getTypeIcon(type)} style={{ marginRight: '0.5rem' }} />
        ...
        {showLearnMore && (href || onClick) && (
          <ButtonContainer>
            ...
            >
              {learnMoreText}
              <i className="pi pi-arrow-right" style={{ marginLeft: '0.5rem' }} />
            </Button>
          </ButtonContainer>
        )}
      </CardBody>
