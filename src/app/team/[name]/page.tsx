...
export default function TeamMemberPage() {
        ...
        <div className="max-w-4xl mx-auto">
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
            <div className="text-center py-8">
              <h1 className="text-2xl font-bold text-sage-green-800 mb-4">Team Member Not Found</h1>
  ...
  const memberClasses = classesData.filter(c => c.instructor === memberData.name);
      ...
      <div className="p-4">
        <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-md h-full">
          <div className="p-4">
            ...
            <p className="text-sm text-sage-green-600 mb-3 line-clamp-3">{classItem.description}</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-sage-green-50 to-tan-100 p-4">
      <div className="max-w-6xl mx-auto">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap gap-2 mb-4">
                  <Tag
                    ...
                    className="text-sm"
                  />
                  <Tag value={memberData.profession} severity="info" className="text-sm" />
                </div>
                <h1 className="text-3xl lg:text-4xl font-bold text-sage-green-800 mb-4">
                  {memberData.name}
                </h1>
                <p className="text-lg text-sage-green-600 mb-4">{memberData.credentials}</p>
                <p className="text-sage-green-700 leading-relaxed mb-4">{memberData.bio}</p>
                <p className="text-sage-green-700 leading-relaxed">{memberData.longBio}</p>
              </div>
            </div>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="font-semibold text-sage-green-700 mb-2">Experience</h3>
                  <p className="text-sage-green-600">{memberData.experience}</p>
                </div>
                <Divider />
                <div>
                  <h3 className="font-semibold text-sage-green-700 mb-2">Education</h3>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
            <div className="p-6">
              <h2 className="text-2xl font-bold text-sage-green-800 mb-4">Specialties</h2>

              <div className="flex flex-wrap gap-2">
                {memberData.specialties.map((specialty, index) => (
                  <Tag key={index} value={specialty} severity="info" className="text-sm" />
                ))}
              </div>
            </div>
          </Card>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
            <div className="p-6">
              <h2 className="text-2xl font-bold text-sage-green-800 mb-4">Contact</h2>

              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium text-sage-green-700">Email</label>
                  <p className="text-sage-green-600">{memberData.contact.email}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-sage-green-700">Phone</label>
                  <p className="text-sage-green-600">{memberData.contact.phone}</p>
                </div>
                {memberData.contact.website && (
                  <div>
                    <label className="block text-sm font-medium text-sage-green-700">Website</label>
                    <a
